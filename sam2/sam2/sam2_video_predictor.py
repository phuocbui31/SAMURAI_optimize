...
class SAM2VideoPredictor(SAM2Base):
    ...
    def append_frame_as_cond_frame(self, inference_state, frame_idx):
        """
        Chuyển một non-cond frame thành conditioning frame.
        Dùng trước khi release_old_frames() để đảm bảo luôn có
        conditioning frame gần frame hiện tại.
        """
        output_dict = inference_state["output_dict"]

        if frame_idx not in output_dict["non_cond_frame_outputs"]:
            return  # Frame không tồn tại hoặc đã là cond frame
