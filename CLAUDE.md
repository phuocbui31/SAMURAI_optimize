...
**Note (2026-05-02 fix):** `inference_time_ms` was empty in *every* CSV row of Stage 1 preload runs prior to 2026-05-02 because `frame_extras_state["inference_time_ms"]` was initialized to `None` and never updated in the frame loop. Fix: `samurai/scripts/main_inference_preload.py` now snapshots `time.perf_counter()` at the end of each iteration and writes the elapsed milliseconds back to the provider state (lag-1 — same precedent as `prev_predicted_bbox`/`prev_predicted_iou`). Row 0 stays empty by design; rows 1..N carry the wall-clock duration of full per-frame iteration N-1. CSVs from runs before this date will have an empty `inference_time_ms` column — re-run Stage 1 if FPS data is needed for analysis. See `docs/superpowers/specs/2026-05-02-fix-inference-time-ms-design.md`.
